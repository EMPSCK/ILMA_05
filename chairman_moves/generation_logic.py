...
async def get_ans(data):
    ...
    for i in group_list:
        ...
        if final_status == 1:
            all_judges_list = await floor_filter(all_judges_list_start, group_floor)
            all_zgs_list = await floor_filter(all_zgs_list_start, group_floor)
            group_all_judges_list = all_judges_list.copy()
            zgs_list_generation = all_zgs_list.copy()
        ...
        if len(zgs_list_generation) >= zgs_number_to_have:
            while len(zgs_end_list) < zgs_number_to_have:
                if len(zgs_list_generation) > 0:
                    ...
                    #Гендерная проверка
                    if need_num == 1 and 0 not in zgs_genders and i[3] == 1:
                        zgs_list_generation.pop(zgs_random_choice['id'], None)
                    else:
                        ...
                        zgs_list_generation = await delete_club_from_judges(zgs_list_generation,
                                                                            zgs_random_choice['Club'],
                                                                            zgs_random_choice['City'])
...
#отфильтровать списо судей по площадкам
async def floor_filter(j_list, floor):
    j_l = j_list.copy()
    for i in j_list:
        if j_list[i]['floor'] == floor or j_list[i]['floor'] is None:
            pass
        else:
            j_l.pop(i, None)
    return j_l
...
async def kill_yana(all_judges_list):
    all_judges_list_1 = all_judges_list.copy()
    for i in all_judges_list:
        if all_judges_list[i]['workCode'] != 0:
            all_judges_list_1.pop(i, None)
    return all_judges_list_1
